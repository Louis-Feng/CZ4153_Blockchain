...
contract("DEX", (accounts) => {
  ...
  before("add some buy/sell orders (price 15: 500)  (price 30: 300, 1, 2)", async () => {
      ...
      await dex.storeOrder(tokenA.address,true, 30, 3, accounts[5]);
      ...
      await dex.storeOrder(tokenA.address,false, 30, 3, accounts[5]);
  });

    describe("remove a sell offer: only offer at this price", async () => {

      it("remove sell offer", async () => {
        let result = await dex.removeOrder(token.address, tokenA.address, true, 15, 1, {from: accounts[5]});
        ...
        assert.equal(orders[0].length, 4)
    ...
    describe("remove a sell offer: multiple offer at this price", async () => {
      ...
      it("User only has one offer at this price", async () => {
        await dex.removeOrder(token.address, tokenA.address, true, 30, 2, {from: accounts[4]});
        ...
        assert.equal(offerList[0], 3);
        assert.equal(offerList[1], 1);
        assert.equal(offerList[2], 4);
    ...
    describe("remove a sell offer: multiple offer at this price", async () => {
          ...
          it("User has multiple offer at this price", async () => {
            await dex.removeOrder(token.address, tokenA.address, true, 30, 4, {from: accounts[5]});
            let result = await dex.getOrderBookInfo(tokenA.address, true);
            assert.equal(result[0], "sell");
            assert.equal(result[1], 1);
            assert.equal(result[2], 30);
    ...
    describe("remove a buy offer: only offer at this price", async () => {
      ...
      it("remove buy offer", async () => {
        let result = await dex.removeOrder(token.address, tokenA.address, false, 15, 1, {from: accounts[5]});
    ...
    describe("remove a buy offer: multiple offer at this price", async () => {
      ...
      it("User only has one offer at this price", async () => {
        await dex.removeOrder(token.address, tokenA.address, false, 30, 2, {from: accounts[4]});
    ...
    describe("remove a buy offer: multiple offer at this price", async () => {
          ...
          it("User has multiple offer at this price", async () => {
            await dex.removeOrder(token.address, tokenA.address, false, 30, 4, {from: accounts[5]});
            let result = await dex.getOrderBookInfo(tokenA.address, false);
            assert.equal(result[0], "buy");
            assert.equal(result[1], 1);
            assert.equal(result[2], 30);
